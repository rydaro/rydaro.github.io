...
This is a tutorial for using propensity score methods for comparative effectiveness research. Example using medical claims data with R code provided at each step.
...
###Lack fo Reporting and Reproducibility
...
```{r stratification, eval=FALSE, include=FALSE}
library(gtools)
...
est_out <-function(model_object){
  out<-matrix(nrow=1,ncol=3)
    out[1,1] <-round(summary(model_object)$coefficients[2,1],2)   
    out[1,2] <-round(summary(model_object)$coefficients[2,2],2)
    out[1,3] <-round(dim(model_object$data)[1]/2834,3)
  colnames(out) <-c('estimate','std','weight')
  rownames(out)<-names(model_object$coefficients[2])
  print(out)
}
